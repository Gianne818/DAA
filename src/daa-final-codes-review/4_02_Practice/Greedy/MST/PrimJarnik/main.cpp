...
Prim-Jarník: grows tree from start node, picks smallest edge connecting tree to new node (priority queue).
    ...
    do {
        ...
        switch (op) {
            ...
            case 'm':
                char start;
                cin >> start;
                mst = graph->primJarnik(start);
                if (mst == nullptr) {
                    cout << "No MST (graph disconnected)" << endl;
                } else {
                    cout << "Prim-Jarník MST Edges:" << endl;
